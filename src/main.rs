...
/// File->Open option
fn on_menu_open(win : &Window, lbl: &Rc<RefCell<Label>>) {
...
fn process_file (filename : PathBuf, win : &Window, lbl: &Rc<RefCell<Label>>) {
...
fn init_menus (window : &Window, msg: &Rc<RefCell<Label>>) -> MenuBar {
    let menu = Menu::new();
...
fn init_toolbar(window : &Window, msg: &Rc<RefCell<Label>>) -> Toolbar {
...
fn main() {
    ...
    let file_counter = Rc::new(RefCell::new(Label::new("No file counted")));

    // create the application menu
    let menu_bar = init_menus(&window, &file_counter);
    v_box.pack_start(&menu_bar, false, false, 0);

    // followed by the toolbar
    let tool_bar = init_toolbar(&window, &file_counter);
